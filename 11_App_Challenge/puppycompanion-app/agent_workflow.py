...
class AgentWorkflow:
    ...
    def get_final_response(self, result):
        ...
        # Check all messages to find tool indicators and track state changes
        next_tool_transitions = []
        for msg in result.get("messages", []):
            if isinstance(msg, AIMessage):
                content = msg.content
                if "[Using RAG tool]" in content:
                    tool_used = "rag"
                    sources = ["The book: \"Puppies for Dummies\""]
                    logger.info(f"Detected RAG tool usage in message")
                elif "[Using Tavily tool]" in content:
                    tool_used = "tavily"
                    logger.info(f"Detected Tavily tool usage in message")
                    # Extract URLs or titles from Tavily results
                    tavily_content = content.replace("[Using Tavily tool] ", "")
                    if "Internet search results:" in tavily_content:
                        import re
                        # Try to extract source titles
                        titles = re.findall(r"\d+\.\s+([^:]+):", tavily_content)
                        if titles:
                            sources = titles[:3]  # Limit to first 3 sources
                            logger.info(f"Extracted Tavily sources: {sources}")
        
        # Log the workflow result state for debugging
        if "next_tool" in result:
            logger.info(f"Final next_tool state: {result['next_tool']}")
        
        # If we couldn't determine the tool from messages, use next_tool if available
        if not tool_used and "next_tool" in result:
            next_tool = result["next_tool"]
            if next_tool in ["need_tavily", "tavily_tool"]:
                tool_used = "tavily"
                sources = ["Internet search results"]
                logger.info(f"Determined tool from next_tool state: {tool_used}")
            elif next_tool == "rag_tool":
                tool_used = "rag"
                sources = ["The book: \"Puppies for Dummies\""]
                logger.info(f"Determined tool from next_tool state: {tool_used}")
        ...
        for msg in reversed(result["messages"]):
            if isinstance(msg, AIMessage) and not msg.content.startswith("[Using"):
                ...
                logger.info("Found final response message")
                break
        ...
        if not response_content:
            for msg in reversed(result["messages"]):
                if isinstance(msg, AIMessage):
                    response_content = msg.content.replace("[Using RAG tool] ", "").replace("[Using Tavily tool] ", "")
                    logger.info("Using fallback response message")
                    break
        
        if not response_content:
            response_content = "I couldn't generate a response."
            logger.info("No response content found")
        
        # Look for signals in the final response content to determine tool used
        if not tool_used:
            if "landmine" in response_content.lower() or "explosive" in response_content.lower():
                # This is likely Tavily as RAG wouldn't have this info about landmines
                tool_used = "tavily"
                sources = ["Internet search results"]
                logger.info("Detected Tavily usage from response content (landmine/explosive keywords)")
        
        # Return the response content and source information
        logger.info(f"Final response tool: {tool_used}, sources: {sources}")
        return {
