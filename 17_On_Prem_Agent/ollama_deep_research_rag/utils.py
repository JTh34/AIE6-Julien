...
@traceable
def setup_qdrant_vectorstore(config: Configuration):
    """Initialize the QDrant connection and the VectorStore."""
    try:
        # QDrant client
...
@traceable
def search_local_knowledge(vectorstore, query: str, config: Configuration):
    ...
    try:
        ...
        docs = vectorstore.similarity_search_with_score(
            query, 
            k=config.max_rag_results
        )
        
        # Filter by confidence
        relevant_docs = [
            (doc, score) for doc, score in docs 
            if score >= config.min_rag_confidence
        ]
        ...
        return {"results": formatted_results, "found": True}
...
def format_rag_results(rag_results):
    ...
    if not rag_results["found"]:
        return "No relevant results found in the local knowledge base."
    
    formatted = "Local knowledge base results:\n\n"
    for i, result in enumerate(rag_results["results"], 1):
        formatted += f"Local source {i} (Score: {result['score']:.2f}):\n"
